...
export default function CanvasPreview({ nodes, selectedId, onSelect }: Props) {
  return (
    <div className="w-full h-full relative">
      {nodes.map((n) => (
        <RenderNode key={n.id} node={n} selectedId={selectedId} onSelect={onSelect} />
      ))}
    </div>
  )
}

function RenderNode({ node, selectedId, onSelect }: { node: Node; selectedId?: string; onSelect: (id: string) => void }) {
    ...
    case 'Div':
      return (
        <div {...commonProps}>
          {node.text}
          {node.children?.map((c) => (
            <RenderNode key={c.id} node={c} selectedId={selectedId} onSelect={onSelect} />
          ))}
        </div>
      )
    case 'Image':
