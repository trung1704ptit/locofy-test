...
export function traverse(nodes: Node[], visit: (node: Node, parentId?: string) => void, parentId?: string) {
...
export function updateNodeById(nodes: Node[], id: string, updater: (n: Node) => Node): Node[] {
  return nodes.map((n) => {
    if (n.id === id) {
      return updater({ ...n })
    }
...
interface Node {
  id: string
  type: string // 'Div' | 'Image' | 'Input' | 'Button'
  children?: Node[]
}
...
export function nodeToStyle(node: Node): CSSProperties {
  ...
  if (node.type === 'Div') {
    if (node.text) {
      ...
      ;(style as any).alignItems = (style as any).alignItems || 'center'
      ;(style as any).justifyContent = (style as any).justifyContent || 'center'
    }
    if (!('borderRadius' in style)) {
      ;(style as any).borderRadius = 8
    }
    ;(style as any).padding = (style as any).padding || 8
  }
